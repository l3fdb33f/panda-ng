...
/// Read a NUL-terminated string from guest virtual memory. panda-ng's panda-rs
/// has no `CPUState::mem_read_string`; read byte-by-byte (small reads avoid
/// page-crossing failures) up to a sane cap.
fn read_cstr(cpu: &mut CPUState, ptr: target_ptr_t) -> String {
    let mut out = Vec::new();
    for i in 0..255u64 {
        match panda::mem::virtual_memory_read(cpu, ptr.wrapping_add(i as target_ptr_t), 1) {
            Ok(b) if !b.is_empty() && b[0] != 0 => out.push(b[0]),
            _ => break,
        }
    }
...
impl File {
    fn read_dentry_name(&self, cpu: &mut CPUState, is_mnt: bool) -> Option<String> {
        ...
        while current_dentry_parent != current_dentry {
            ...
            let name = read_cstr(cpu, name_ptr);
